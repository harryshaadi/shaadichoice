<?php
	...
	function getUserChatDtl($conversation_id){
		global $db;
		...
		if($q > 0){
			...
			foreach($prof as $m) {
				...
				echo "<div class='message'><div class='img-con'><img src='{$user_form_img}'></div><div class='text-con'><a href='#''>{$user_form_username}</a><p>{$message}</p><p>{$chat_date}</p></div></div><hr>";
